...
impl MigrationTrait for Migration {
  async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
    ...
    let insert_permissions = Query::insert()
      ...
      .values_panic(["Metadata.Edit".into(),"Modify titles, descriptions, release dates, and cast information for any media item.".into(), true.into()])
      .values_panic(["Metadata.Artwork".into(),"Upload or change posters, backdrops, and logo images for movies or shows.".into(), true.into()])
      .values_panic(["Metadata.Lock".into(),"Prevent the automated scanner from overwriting manual changes to specific media items.".into(), true.into()])
      .values_panic(["Metadata.View".into(),"Allow viewing extended metadata details (NFO, raw paths, IDs).".into(), true.into()])
      ...
      .values_panic(["User.ManageRoles".into(),"ElevatUser.ManagePermse or demote users by assigning or removing specific Roles.".into(), true.into()])
      .values_panic(["User.ManagePerms".into(),"This permission allows administrators to directly modify permission metadata within the system, or to elevate or demote users by assigning or removing specific roles.".into(), true.into()])
